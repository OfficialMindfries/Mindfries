...
**This is the deep-dive version.** [`task.md`](task.md) is the short,
plain-language status page — a table plus a few short lists, meant to be
read in a couple of minutes. This file is everything behind it: full
security findings with severity and file:line citations, the complete API
endpoint inventory, exactly what was verified live and how, and the
pass-by-pass history of what changed. Kept because deleting it would lose
real, checked-against-the-code detail (several GitHub issues cite findings
here by ID — B1, C1, F1, and so on) — not because `task.md` is incomplete
without it. Read `task.md` first; come here for the "why" and "exactly
how do you know that" behind any line in it.
...
superseded by this file (see the note at the top of each).
