// The signed session cookie for the Company Portal.
//
...
// in both, so one implementation covers both and there is only one place
// where a session is judged valid. Same approach as internal-admin's
// lib/auth/session.ts, its own SESSION_SECRET — the two apps never share a
// session secret, so a signed-in admin can't forge a company session or vice
// versa (see .env.example).
//
...
export const SESSION_COOKIE = "mf_company";
 ...
 * — which is the safe direction to fail. Never defaulted to a constant.
 */
...
export async function readSession(token: string | undefined, secret: string | null): Promise<Session | null> {
  ...
  try {
    ...
    if (typeof claims.companyId !== "string" || !claims.companyId) return null;
    return claims;
  } catch {
    return null;
  }
