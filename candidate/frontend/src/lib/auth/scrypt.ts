...
// Password hashing, as plain Node — ported from internal-admin's
// lib/auth/scrypt.ts verbatim (same reasoning applies here unchanged: scrypt
// is memory-hard, which is the point, and this module stays unguarded so
// scripts/*.mts (outside Next) can hash a password too, while the app itself
// goes through the guarded ./password.ts).
//
 ...
 * "no such account" and "wrong password" take comparably long. Without it,
 * the login page quietly answers "does this email have an account here?".
 */
