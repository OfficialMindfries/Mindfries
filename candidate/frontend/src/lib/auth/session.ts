// The signed session cookie — ported from internal-admin's lib/auth/session.ts,
// same mechanism, this app's own cookie name and secret.
//
...
// where a session is judged valid.
//
// The cookie carries the claims and a signature over them. It is signed, not
// encrypted — so never put anything in here a candidate shouldn't read back.
...
export const SESSION_MAX_AGE = 60 * 60 * 24 * 14; // 14 days — a candidate shouldn't be signed out mid-assessment-search
 ...
 * because a known secret means anyone can mint a session for any candidate.
 */
